                ...
                >
                  ...
                  <div className="flex flex-1 items-center justify-between">
                    <div>
                      ...
                      <p className="mt-1 flex items-center gap-2 text-sm text-muted">
                        <span className="inline-flex h-6 w-6 items-center justify-center rounded-full bg-zinc-800 border border-amber-600/20 font-display text-[10px] text-amber-500">
                          {(order.seller.profile?.username ?? order.seller.name ?? "?").charAt(0).toUpperCase()}
                        </span>
                        {order.seller.profile?.username ?? order.seller.name}
                      </p>
                    </div>
                    <div className="flex flex-col items-end gap-2">
                      ...
                      <span
                        className={`rounded-full px-2.5 py-0.5 text-xs font-medium ${STATUS_COLORS[order.status] ?? "bg-[var(--glass-card-bg)] text-[var(--text-muted)] border border-[var(--divider)]"}`}
                      >
